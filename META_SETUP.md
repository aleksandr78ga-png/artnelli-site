...
После подключения сайт принимает новые товарные публикации канала и изменения подписей. Действует только правило наличия публикации: новый товарный пост добавляется в каталог, а удалённый пост полностью удаляется с сайта. Статус «Продан» не используется. Поскольку Telegram Bot API не сообщает боту об удалении записи, сайт дополнительно проверяет существование товарных публикаций в канале.
