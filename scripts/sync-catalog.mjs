...
const stateFile = path.join(rootDir, "catalog-sync-state.json");
const telegramSource = "https://t.me/nelli_leotards";
...
async function main() {
  ...
  const removedLocalCount = localProducts.filter((product) =>
    removedIds.has(Number(product.id)),
  ).length;
  if (removedLocalCount > removalLimit) {
    throw new Error(
      `Защитная остановка: Telegram удаляет ${removedLocalCount} карточек из ${localProducts.length}`,
    );
  }
  ...
  if (products.length < localProducts.length - removalLimit) {
    throw new Error(
      `Защитная остановка: каталог уменьшился с ${localProducts.length} до ${products.length}`,
    );
  }
  const ids = products.map((product) => Number(product.id));
  if (ids.some((id) => !Number.isFinite(id)) || new Set(ids).size !== ids.length) {
    throw new Error("В итоговом каталоге есть неверные или повторяющиеся ID");
  }
  for (const product of products) {
    if (!product.name || !["new", "used"].includes(product.condition)) {
      throw new Error(`Неполная карточка товара ${product.id}`);
    }
    if (!Array.isArray(product.photos) || product.photos.length === 0) {
      throw new Error(`У товара ${product.id} нет фотографий`);
    }
  }
  ...
  const nextStateSource = `${JSON.stringify(
    {
      ...
      newCount: products.filter((product) => product.condition === "new").length,
      usedCount: products.filter((product) => product.condition === "used").length,
      removedIds: [...removedIds].sort((left, right) => left - right),
    },
    null,
    2,
  )}\n`;
  const currentStateSource = (await fileExists(stateFile))
    ? await readFile(stateFile, "utf8")
    : "";
  if (currentStateSource !== nextStateSource) {
    await writeFile(stateFile, nextStateSource);
  }
